...
node* avl::rotateRight(node* j){
   ...
   */
	...
	node * k=j->getLeft();
	j->setLeft(k->getRight());
	if(k->getRight())
		k->getRight()->setParent(j);
	k->setParent(j->getParent());
	if(j->getParent())
	{
		if(j->getParent()->getRight()==j)
			j->getParent()->setRight(k);
		else
			j->getParent()->setLeft(k);
	}
	j->setParent(k);
	k->setRight(j);	
	return k;
...
node* avl::rotateLeft(node* j){
	node * k=j->getRight();
	k->setParent(j->getParent());
	j->setRight(k->getLeft());
	if(k->getLeft())
	k->getLeft()->setParent(j);
	if(j->getParent())
	{
		if(j->getParent()->getRight()==j)
			j->getParent()->setRight(k);
